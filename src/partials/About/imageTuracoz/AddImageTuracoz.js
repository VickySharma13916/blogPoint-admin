...
const AddImageTuracoz = ({
  ...
  const onSubmit = async (data) => {
    const newData = {
      ...data,
      turacozImage: brandimage,
    };
    const result = await AddImageTuracoz(newData);
    if (result.status === 200) {
      history.push("/about");
    }
  };
      ...
      header={
        <>
          <IconButton
            ...
            className="p-0 me-2"
          />
          <h6 className="mb-0">Add Image Slider </h6>
        </>
      }
    >
...
const mapDispatchToProps = (dispatch) => ({
  AddImageTuracoz: (payload) => dispatch(performPostImageSliderAbout(payload)),
  performImageUpdate: (payload) =>
    dispatch(performteamImageImageSliderAbout(payload)),
});

export default connect(mapStateToProps, mapDispatchToProps)(AddImageTuracoz);
