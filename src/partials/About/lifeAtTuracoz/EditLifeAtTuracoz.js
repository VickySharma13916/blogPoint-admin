...
import {
  performUpdateLifeAtTuracozTeam,
...
const EditLifeAtTuracoz = ({
  ...
  const {
    ...
    defaultValues: {
      turacozteamName: userData?.turacozteamName,
      turacozteamDesignation: userData?.turacozteamDesignation,
      turacozteamAbout: userData?.turacozteamAbout,
      turacozteamImage: userData?.turacozteamImage,
    },
  });
  ...
  const onSubmit = async (data) => {
    const newData = {
      ...data,
      turacozteamImage: brandimage ? brandimage : userData?.turacozteamImage,
      id: userData?._id,
    };
    const result = await EditLifeAtTuracoz(newData);
    if (result.status === 200) {
      history.push("/about");
    }
  };
            ...
            <div className="image-uploader-container">
              ...
              <div className="image-container">
                {brandimage ? (
                  ...
                  <Avatar
                    src={BACKEND_URL + userData?.turacozteamImage}
                    name={userData?.turacozteamName}
                    size="100"
...
const mapDispatchToProps = (dispatch) => ({
  EditLifeAtTuracoz: (payload) =>
    dispatch(performUpdateLifeAtTuracozTeam(payload)),
  performImageUpdate: (payload) =>
    dispatch(performteamImageLifeAtTuracozTeam(payload)),
});

export default connect(mapStateToProps, mapDispatchToProps)(EditLifeAtTuracoz);
