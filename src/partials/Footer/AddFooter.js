...
import { performPostFooter } from "../../redux/actionCreators/footerCreators";

const AddFooter = ({ submitting, handleAddFooter, history }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const result = await handleAddFooter(newData);
      ...
      header={
        <>
          <IconButton
            ...
            className="p-0 me-2"
          />
          <h6 className="mb-0">Add Footer</h6>
        </>
      }
    >
      {preview ? (
...
const mapDispatchToProps = (dispatch) => ({
  handleAddFooter: (payload) => dispatch(performPostFooter(payload)),
});

export default connect(mapStateToProps, mapDispatchToProps)(AddFooter);
