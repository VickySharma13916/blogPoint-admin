...
const AddWebinar = ({
  ...
  const PostwebinarImages = async (e) => {
    ...
    const result = await handleAddwebinarImageCourse(formData);
    if (result.status === 200) {
  ...
  const onSubmit = async (data) => {
    ...
    const result = await handleAddWebinar(newData);
    if (result.status === 200) {
      history.goBack();
    }
  };
      ...
      header={
        <>
          <IconButton
            ...
            className="p-0 me-2"
          />
          <h6 className="mb-0">Add Webinar</h6>
        </>
      }
    >
      <Form onSubmit={handleSubmit(onSubmit)}>
        <Row>
          ...
          <Col xs={12}>
            <div className="brandimage mb-2">Select Weninar Type</div>
            <Select
...
const mapDispatchToProps = (dispatch) => ({
  handleAddWebinar: (payload) => dispatch(performPostWebinar(payload)),
  handleAddImageCourse: (payload) =>
    dispatch(performPostImageWebinar(payload, "webinarImage")),
  handleAddwebinarImageCourse: (payload) =>
    dispatch(performPostImageWebinar(payload, "webinarTimeLineImage")),
});

export default connect(mapStateToProps, mapDispatchToProps)(AddWebinar);
