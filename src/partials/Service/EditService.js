...
import { performUpdateService } from "../../redux/actionCreators/serviceCreators";
const schema = yup
  .object({
  ...
  .required();
const EditService = ({ submitting, handleEditService, history }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      serviceLink: userData?.serviceLink,
  ...
  const [content, setContent] = useState(userData?.serviceDescription);
  ...
  const onSubmit = async (data) => {
    ...
    const newData = {
      ...data,
      id: userData?._id,
      serviceDescription: content ? content : userData?.serviceDescription,
    };
    const result = await handleEditService(newData);
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Row>
              ...
              <Col xs={12}>
                ...
                <Editor
                  content={content}
                  defaultValue={userData?.serviceDescription}
                  setContent={setContent}
                  placeholder={"Start Typing Service Content here..."}
                />
                {contentmssg && (
                  <p
...
const mapDispatchToProps = (dispatch) => ({
  handleEditService: (payload) => dispatch(performUpdateService(payload)),
});
